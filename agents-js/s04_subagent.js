 ...
 * @param {string} prompt - The task prompt for the subagent
 * @returns {Promise<string>} - Summary of the subagent's work
 */
async function runSubagent(prompt) {
    // Fresh context - start with empty messages
    const subMessages = [
...
/**
 * Parent agent loop with subagent delegation
    ...
    while (true) {
        ...
        for (const toolCall of assistantMessage.tool_calls) {
            ...
            if (toolName === "task") {
                ...
                console.log(`> task (${desc}): ${args.prompt.slice(0, 80)}`);
...
export { agentLoop, runSubagent, runBash, runRead, runWrite, runEdit };